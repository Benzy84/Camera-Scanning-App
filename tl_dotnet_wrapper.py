...
class TL_Camera(object):
    ...
    def frame_to_array(frame) -> np.ndarray:
        ...
        try:
            ...
            # Convert image to numpy array directly
            ret_array = np.array(image, dtype=np.uint16)

            ret_array = ret_array.reshape(frame.get_ImageData().get_Height_pixels(),
                                          frame.get_ImageData().get_Width_pixels())
            return ret_array
    ...
    def set_exposure_time_us(self, time_us):
        try:
            self.camera.ExposureTime_us = time_us
        except Exception as exception:
    ...
    def set_roi_binning(self, x=0, y=0, width=3296, height=2472, binX=1, binY=1):
    ...
    def get_data_rate(self):
        try:
            if self.camera.DataRate == DataRate.ReadoutSpeed20MHz:
                return "20MHz"
            elif self.camera.DataRate == DataRate.ReadoutSpeed40MHz:
                return "40MHz"
            elif self.camera.DataRate == DataRate.FPS30:
                return "FPS30"
            elif self.camera.DataRate == DataRate.FPS50:
                return "FPS50"
    ...
    def get_exposure_time_ms(self):
        try:
            return int(0.001 * self.camera.ExposureTime_us)
        except BaseException as be:
            raise CameraExceptionError("Error: GetExposureTime failed\n" + str(be))
