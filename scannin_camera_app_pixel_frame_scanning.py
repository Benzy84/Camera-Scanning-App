...
class galvo_scanning_thread(QThread):
    ...
    def calculate_total_points_to_scan(self):
        ...
        if self.galvo.scanning_type == 'Circular':
            ...
            total_points = 2 * len(x_range) + 2 * len(y_range)  # Sum of all four edges
        return total_points + 1

    def scan_point(self, x_voltage, y_voltage, scanned_points, last_n_times, start_time_for_this_point):
        ...
        return scanned_points, last_n_times, start_time_for_this_point
    ...
    def run(self):
        ...
        if self.galvo.scanning_type == 'Circular':
            ...
            scanned_points, last_n_times, start_time_for_this_point = self.scan_point(0, 0, scanned_points,
                                                                                      last_n_times,
                                                                                      start_time_for_this_point)
            # Scan the top edge
            y_pixel = 0.5 * pixel_height
            for x_pixel in x_pixels:
                x_voltage, y_voltage = self.pixel_to_voltage(x_pixel, y_pixel)
                if self.galvo.scanning:
                    start_time_for_previous_point = start_time_for_this_point
            ...
            # Scan the right edge
            x_pixel = -0.5 * pixel_width
            for y_pixel in (y_pixels):  # Start from top and go downwards
                x_voltage, y_voltage = self.pixel_to_voltage(x_pixel, y_pixel)
                if self.galvo.scanning:
                    start_time_for_previous_point = start_time_for_this_point
                    start_time_for_this_point = time.time()
            ...
            y_pixel = -0.5 * pixel_height
            for x_pixel in reversed(x_pixels):  # Start from right and go leftwards
                x_voltage, y_voltage = self.pixel_to_voltage(x_pixel, y_pixel)
                if self.galvo.scanning:
                    start_time_for_previous_point = start_time_for_this_point
                    start_time_for_this_point = time.time()
            ...
            # Scan the left edge
            x_pixel = 0.5 * pixel_width
            for y_pixel in reversed(y_pixels):  # Start from bottom and go upwards
                x_voltage, y_voltage = self.pixel_to_voltage(x_pixel, y_pixel)
                if self.galvo.scanning:
                    start_time_for_previous_point = start_time_for_this_point
                    start_time_for_this_point = time.time()
