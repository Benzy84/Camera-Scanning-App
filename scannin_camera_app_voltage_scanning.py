...
class Camera_Application(QMainWindow):
    ...
    def create_scanning_controls_ui(self, main_layout):
        ...
        # Scanning Radius
        scanning_radius_label = QLabel("Scanning Radius [Volt]")
        ...
        # Scanning height
        scanning_height_label = QLabel("Scanning Height [Volt]")
        ...
        # Scanning Width
        scanning_width_label = QLabel("Scanning Width [Volt]")
        ...
        # Step Size
        step_size_label = QLabel("Step Size [Volt]")
        ...
        if self.galvo.scanning_type == 'Circular':
            ...
            for x_voltage in x_range:
                for y_voltage in y_range:
                    if x_voltage ** 2 + y_voltage ** 2 <= scanning_radius ** 2 and self.galvo.scanning:
            ...
            for x_voltage in x_range:
                col += 1
                row = -1
                # if int((x_voltage + 0.5 * scanning_width) / step_size) % 2 == 0:  # Check if it's an even row
                #     y_iterable = y_range
                # else:  # If it's an odd row, reverse the direction
                #     y_iterable = reversed(y_range)
                # for y_voltage in y_iterable:
                for y_voltage in y_range:
                    row += 1
                    if self.galvo.scanning:
                        start_time_for_previous_point = start_time_for_this_point
